...
import { assignBodySchema } from '@/lib/zod'
...
export async function POST(
  ...
  try {
    // Acquire Redis lock
    lockValue = await acquireLock(`shift:${shiftId}:assign`, 10)
    if (!lockValue) {
    ...
    // Create assignment and update shift status
    ...
    if (lockValue) {
      await releaseLock(`shift:${shiftId}:assign`, lockValue)
    }
  }
}
